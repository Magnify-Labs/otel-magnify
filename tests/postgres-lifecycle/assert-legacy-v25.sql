...
BEGIN
    ...
    IF goose_version <> 25 THEN
        RAISE EXCEPTION 'Goose version mismatch: got %, expected 25', goose_version;
    END IF;
...
$collector$
    ...
    IF (SELECT COUNT(*) FROM workloads
        WHERE id = 'upgrade-marker-workload'
          ...
          AND remote_config_status::jsonb = '{"status":"failed","config_hash":"upgrade-fixture-hash","error_message":"collector failed: SECRET_TOKEN=fixture-only","updated_at":"2025-01-02T03:05:06Z"}'::jsonb
        ...
        RAISE EXCEPTION 'workload fixture data, metadata, or raw legacy status mismatch';
    END IF;
    ...
    IF POSITION('SECRET_TOKEN=fixture-only' IN (
        SELECT remote_config_status FROM workloads WHERE id = 'upgrade-marker-workload'
    )) = 0 THEN
        RAISE EXCEPTION 'raw legacy marker is absent';
    END IF;
END
$assert$;
