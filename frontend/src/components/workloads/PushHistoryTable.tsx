...
export default function PushHistoryTable({ workloadId }: Props) {
  ...
  const activeKnownGoodHash = knownGood?.config_id ?? history.find((row) => row.is_last_known_good)?.config_id
  ...
  const markKnownGoodMutation = useMutation({
    mutationFn: (row: WorkloadConfig) =>
      workloadsAPI.markKnownGood(workloadId, row.config_id, {
        ifCurrentKnownGood:
          activeKnownGoodHash && activeKnownGoodHash !== row.config_id ? activeKnownGoodHash : undefined,
        replaceReason:
              ...
              <tr key={rowKey}>
                ...
                <td>
                  ...
                  <button
                    ...
                    title={rollbackDisabledReason || t('workloads.config.versioning.rollback_button')}
                  >
                  ...
                  {isKnownGood ? (
                    <button
                      ...
                      title={canPushConfig ? 'Clear Last known-good' : 'Requires workload:push_config permission'}
                    >
                      Clear known-good
                    </button>
                  ) : (
